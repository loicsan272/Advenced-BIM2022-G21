...
def classifyFloors(floors,site_elev,model):
    ...
    for floor in floors:
        ...
        for b in model.by_type('IfcBeam'):
            ...
            if str(level) in lvl:
                FBeams.append(b)
        se ="<b>Walls:</b> Foundation walls: <b>"+str(len(FoundationW))+"</b> Exterior walls: <b>"+str(len(ExteriorW))+"</b> Party walls: <b>"+str(len(Party_walls))+"</b><br> <b>Slabs: </b> Roof: <b>"+str(len(Roof))+"</b> Finish floors: <b>"+str(len(FinishF))+"</b> Exterior Floors: <b>"+str(len(ExteriorF))+"</b><br><b> Beams:</b> <b>"+str(len(FBeams))+"</b>"
        se = se.replace("<b>0</b>","0")
        type = "floor_upper"
